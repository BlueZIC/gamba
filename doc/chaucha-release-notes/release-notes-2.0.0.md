Monedero version 2.0.0 is now available from:
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/proyecto-gamba/monedero/issues>
...
updates will be released anymore. Without security updates, using a gamba
...
ECDSA signatures inside Gamba transactions now use validation using
...
Previous versions of Monedero had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Monedero was to raise the default minimum
relay fee.

Monedero 0.13.2 will have a strict maximum size on the mempool. The
...
Monedero 0.13.2 also introduces new default policy limits on the length and
...
This allows running gambad without having to do any manual configuration.
...
Monedero has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Monedero will relay transactions with
...
entirely in Monedero 0.13, and it is at this time undecided whether the
...
Monedero has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Monedero automatically creates a hidden service to listen on, without
manual configuration. Monedero will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Monedero is listening, and
...
Gambad can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Gamba
Core to use floating fees. By default, Monedero will use floating
fees.
...
At all times, Monedero will cap fees at `-maxtxfee=<x>` (default:
0.10) CHA.
Furthermore, Monedero will never create transactions paying less than
...
`gamba.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /gambarpc>
        ...
        # optional bypass gambad rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< gambarpc:<password>
    </Location>
...
Restarting gambad will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in gamba.conf
...
For example:
    ...
    $ src/gamba-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Monedero code base has
...
- `gamba-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `gamba-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Gamba scripting system that in combination with
...
Gamba transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Monedero.
...
The two phases are distinct in the Monedero GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Gamba wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Gamba block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of gambas they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of gambas being spent is exactly
...
- **Increased security for multisig:** Gamba addresses (both P2PKH addresses
  ...
  believed to be provided by Gamba's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Gamba's current ledger (called the UTXO set),
  ...
  note that Monedero does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Gamba users to individually opt-in to almost any change in the Gamba
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Monedero contributors that may
...
segwit's start date of 1 January 2017 miners can update the Gamba
client to Monedero 0.13.2 to signal for segwit support. When a
...
Since Monedero 0.10.0, nodes have defaulted to only relaying and
...
- `gamba-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `gamba-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
